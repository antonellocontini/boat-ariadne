...
HybridAutomaton create_permissive_controller_proportional(double prop_constant, double heading) {
    ...
    DiscreteEvent mustUnlockLeft("mustUnlockLeft"), mustUnlockRight("mustUnlockRight");
    ...
    controller.new_transition(position|left, unlockLeft, position|proportional, Theta-Theta_c >= -pi/(2*Kp), EventKind::PERMISSIVE);
    controller.new_transition(position|right, unlockRight, position|proportional, Theta-Theta_c <= pi/(2*Kp), EventKind::PERMISSIVE);

    controller.new_invariant(position|left, Theta-Theta_c <= -pi/(2*Kp)+0.01_decimal, mustUnlockLeft);
    controller.new_invariant(position|right, Theta-Theta_c >= pi/(2*Kp)-0.01_decimal, mustUnlockRight);
